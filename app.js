        ...
        function bg_chnge() {
            ...
            if ((19 <= hh && hh <= 24)) {
                ...
                container1.classList.remove('bg')
            )

        }

        function fetchList() {
            fetch('https://opendata.cwa.gov.tw/api/v1/rest/datastore/F-C0032-001?Authorization=CWB-82546646-C2D2-4070-902B-B0974C6F2646')
                .then(function (response) {
                    return response.json();
                })
                .then(function (result) {
                    console.log(result.records.location);
                    let locationList = result.records.location;
                    console.log(locationList);
                    //北區過濾：臺北市、新北市、基隆市、新竹市、桃園市、新竹縣及宜蘭縣
                    let locationListNorth = locationList.filter((item) => {
                    ...
                    console.log('我是北部', locationListNorth);
                    // 中部區域：包括臺中市、苗栗縣、彰化縣、南投縣及雲林縣
                    let locationListCentral = locationList.filter((item) => {
                    ...
                    let locationListSouth = locationList.filter((item) => {
                    ...
                    // 花蓮縣及臺東縣
                    let locationListEast = locationList.filter((item) => {
                    ...
                    let locationListIsland = locationList.filter((item) => {
            ...
            for (i = 0; i < data.length; i++) {
                ...
                <div class="weather_card col-12 col-lg-5 col-md-5 col-sm-12 col-xs-12 p-0"> 
                   <div class="card p-4  py-5 position-relative">    
                       <div class>
                        <h6 class="flex-grow-1 my-3 location_name">${data[i]['locationName']}</h6>
                        <span class="small grey parameter">${data[i]['weatherElement'][0]['time'][0]['parameter']['parameterName']}</span>−
                        <span class="small grey parameter">${data[i]['weatherElement'][3]['time'][0]['parameter']['parameterName']}</span>
                        </div>
                       <div class="d-flex flex-column temp mt-4 mb-3">
                           <h1 class="mb-5 font-weight-bold temp w-100" id="heading"> ${data[i]['weatherElement'][2]['time'][0]['parameter']['parameterName']}&deg;C ~ ${data[i]['weatherElement'][4]['time'][0]['parameter']['parameterName']}&deg;C </h1>
                       ...
                       <div class="d-flex">
                           <div class="temp-details flex-grow-1">
                                <p class="my-1">
                                   ...
                                   <span> ${data[i]['weatherElement'][4]['time'][0]['parameter']['parameterName']} km/h  </span>
                                </p>
                                <p class="my-1 d-flex align-items-center"> 
                                   <img class="me-2"src="./img/rainy.png" height="17px" >
                                   <span class="pop"> ${data[i]['weatherElement'][1]['time'][0]['parameter']['parameterName']}% </span> 
                                </p>
                           </div>
        ...
        function show_img(data) {
            ...
            var hh = today.getHours();
            console.log('hh', hh);
            weather_png.forEach((item, index) => {
